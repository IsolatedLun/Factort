...
<Card padding={1} cubeClass={{ utilClass: 'margin-block-2' }}>
	<Flexy gap={1} collapseOnMobile={true}>
		<a href={WEB_CREATE_POST_WITH_TYPE_URL(communityId, '')} class="[ width-100 ]">
			<TextInput
				label="Create post"
		...
		<Flexy cubeClass={{ utilClass: 'margin-inline-auto' }}>
			<Button to={WEB_CREATE_POST_WITH_TYPE_URL(communityId, 'link')}>
				<Icon>{ICON_LINK}</Icon>
			</Button>
			<Button to={WEB_CREATE_POST_WITH_TYPE_URL(communityId, 'video')}>
				<Icon>{ICON_MEDIA}</Icon>
			</Button>
			<Button to={WEB_CREATE_POST_WITH_TYPE_URL(communityId, 'audio')}>
				<Icon>{ICON_MICROPHONE}</Icon>
			</Button>
			<Button to={WEB_CREATE_POST_WITH_TYPE_URL(communityId, 'images')}>
				<Icon>{ICON_IMAGE}</Icon>
			</Button>

			{#if type !== 'community'}
				<Button cubeClass={{ utilClass: 'margin-inline-start-2' }} to={WEB_CREATE_COMMUNITY_URL}>
